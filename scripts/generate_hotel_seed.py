"""
scripts/generate_hotel_seed.py
──────────────────────────────
Generates a structured hotel_seed.json file for Firebase ingestion.
This version uses the flattened collection-based format.

Layout per floor (24 wide x 12 tall):
  Row 0:   Front wall (exits punched at cols 3, 11, 19)
  Rows 1-3:  Front rooms (10 rooms, cols 1,3,5,...,19)
  Rows 4-7:  Corridor (fully walkable)
  Rows 8-10: Back rooms (10 rooms, cols 1,3,5,...,19)
  Row 11:  Back wall (exits punched at cols 3, 11, 19)

Hotel: 3 Blocks (North, Central, South), 4 floors each = 12 floors total
Each floor: 20 rooms, 6 exits (3 front + 3 back), 3 aid kits

Usage:
    python scripts/generate_hotel_seed.py
    -> writes scripts/hotel_seed.json
"""
import json
import random
import uuid
...
out_path = os.path.join("scripts", "hotel_seed.json")
